...
mod tests {
    ...
    fn test_constant_folding_addition() {
        ...
        let ast = TaggedAst::binary_op(
            "+",
            TaggedAst::literal_number(2.0),
            TaggedAst::literal_number(3.0),
        );

        let result = optimizer.fold_constants(ast);
        assert_eq!(result.tag, "literal_number");
        assert_eq!(result.get_attr_number("value"), Some(5.0));
    }
    ...
    fn test_constant_folding_multiplication() {
        ...
        let ast = TaggedAst::binary_op(
            "*",
            TaggedAst::literal_number(4.0),
            TaggedAst::literal_number(5.0),
        );

        let result = optimizer.fold_constants(ast);
        assert_eq!(result.tag, "literal_number");
        assert_eq!(result.get_attr_number("value"), Some(20.0));
    }
    ...
    fn test_nested_constant_folding() {
        let optimizer = LogicOptimizer::new();
        // (2 + 3) * 4 = 20
        let inner = TaggedAst::binary_op(
            "+",
            TaggedAst::literal_number(2.0),
            TaggedAst::literal_number(3.0),
        );
        let outer = TaggedAst::binary_op("*", inner, TaggedAst::literal_number(4.0));

        let result = optimizer.fold_constants(outer);
        assert_eq!(result.tag, "literal_number");
        assert_eq!(result.get_attr_number("value"), Some(20.0));
    }
    ...
    fn test_no_division_by_zero() {
        ...
        let ast = TaggedAst::binary_op(
            "/",
            TaggedAst::literal_number(10.0),
            TaggedAst::literal_number(0.0),
        );

        let result = optimizer.fold_constants(ast);
        // Should NOT fold division by zero
        assert_eq!(result.tag, "binary_op");
    }
    ...
    fn test_dead_code_elimination() {
        ...
        let block = TaggedAst::new("block").with_children(vec![
            TaggedAst::literal_number(1.0),
            TaggedAst::new("return").with_child(TaggedAst::literal_number(2.0)),
            TaggedAst::literal_number(3.0), // Dead code
            TaggedAst::literal_number(4.0), // Dead code
        ]);

        let result = optimizer.eliminate_dead_code(block);
        let children = result.children.unwrap();
        assert_eq!(children.len(), 2); // Only first two statements remain
    }
}
